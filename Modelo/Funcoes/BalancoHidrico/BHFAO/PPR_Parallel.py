...
def calc_yx(T, Cc, a, serie_PPR, serie_T, lat_rad):
    # start = time.time()
    ...
    Jo = 899.5  # Constante solar 899,5 cal/cm²/dia¹
    radiacao_topo_atmosfera = Jo * DR * (
                (np.pi / 180) * angulo_solar * np.sin(declinacao_solar_r) * np.sin(lat_rad) + np.cos(
            declinacao_solar_r) * np.cos(lat_rad) * np.sin(agulo_solar_r))
    ...
    PPBp = PPBn + PPBc
    PPR_ = (0.265455 * Cc) * PPBp

    y, x = 1287, 1032
    print(PPR_[x][y], T_[x][y])
...
class PPR(AbstractFunction):
    ...
    def __execOperation__(self):
        ...
        while i < n_imagens:
            #print(n_processors)
            if(n_processors>0):
                p = Process(target=calc_yx, args=(serie_T[i], self.Cc, a, serie_PPR, serie_T, lat_rad))
                p.daemon = True
                processos.append(p)
                p.start()
                n_processors -=1
                ...
                for runnin_p in processos:
                    if not runnin_p.is_alive():
                        processos.remove(runnin_p)
                        n_processors +=1
