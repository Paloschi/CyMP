...
class PPR(AbstractFunction):
    ...
    def __execOperation__(self):
        ...
        for T in serie_T:
            ...
            radiacao_topo_atmosfera = Jo * DR * ((np.pi/180) * angulo_solar * np.sin(declinacao_solar_r) * np.sin(lat_rad) + np.cos(declinacao_solar_r) * np.cos(lat_rad) * np.sin(agulo_solar_r))
